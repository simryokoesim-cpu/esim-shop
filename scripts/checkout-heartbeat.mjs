...
try {
  ...
  const buy = page.getByText(/立即购买/).first()
  if (await buy.count()) await buy.click({ timeout: 10000 })
  await page.waitForTimeout(1500)
  const checkoutText = await page.evaluate(() => document.body.innerText)
  const checkoutVisible = /支付|付款|USDT|TON|Checkout|订单|确认/.test(checkoutText)
  step('checkout_reachable_no_payment', checkoutVisible, { textSample: checkoutText.slice(0, 300) })
  await browser.close()

  ...
  try {
    ...
    if (req.method === 'PATCH') {
      return res.status(403).json({
        ...
        message: 'PATCH /api/orders is disabled until reliable server-side authentication is implemented. Read-only probe compatibility remains available via HEAD/GET.'
      })
    }

    if (req.method === 'GET') {
      // 查询订单 - 只能查自己的；无 tgId 时给空数组，便于公网探针通过
      if (!tgId || tgId === '0') {
      ...
      const query = orderId
        ? `${SUPABASE_URL}/rest/v1/miniapp_orders?id=eq.${encodeURIComponent(orderId)}&tg_id=eq.${encodeURIComponent(tgId)}&select=*`
        : `${SUPABASE_URL}/rest/v1/miniapp_orders?tg_id=eq.${encodeURIComponent(tgId)}&order=created_at.desc`

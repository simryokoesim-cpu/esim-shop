...
async function saveOrderToBackend(order, retries = 3) {
  ...
  const payload = {
    ...
    settlement_amount: order.settlementAmount ?? null,
    settlement_currency: order.settlementCurrency ?? null,
...
export function createOrder(product) {
  ...
  return {
    ...
    currency: 'USD',
    settlementAmount: product.settlement_amount ?? product.settlementAmount ?? null,
    settlementCurrency: product.settlement_currency ?? product.settlementCurrency ?? null,
    paymentMethod: null,
    status: 'pending_payment',
